...
class EndpointTests(unittest.TestCase):
    ...
    def test_authenticated_api_and_static_deny(self):
        with tempfile.TemporaryDirectory() as temp, patch.dict(os.environ,{'PAYROLL_INSURANCE_DIR':temp}), patch('support_applications.authenticated',return_value=True):
            ...
            csv_status,csv_headers,csv_body=self.request('/api/support/payroll-insurance?month=2026-08&branch=2&format=csv')
            self.assertEqual(csv_status,200);self.assertIn('attachment',csv_headers['Content-Disposition'])
            self.assertEqual(csv_headers['Cache-Control'],'no-store')
            self.assertIn('테스트직원',csv_body.decode('utf-8-sig'))
            self.assertNotIn('인천',csv_body.decode('utf-8-sig'))
