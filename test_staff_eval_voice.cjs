...
const state = () => ({name:'테스트', role:'요양보호사', branch_label:'인천점', status:'in_progress', consented:true, authenticated:true, input_open:true, voice:{available:true}, progress:{current:1,total:6}, messages:[{role:'assistant',text:'상황 1/6 · 낙상'}]});
...
async function harness(options = {}) {
  ...
  let timerId = 0, stopped = 0, audioClosed = 0, micRequests = 0;
  class Socket {
  ...
  const context = {
    ...
    location:{hash:'#test-token'}, WebSocket:Socket, Blob,
    navigator:{mediaDevices:{async getUserMedia() { micRequests++; return {getTracks:()=>[{stop(){stopped++;}}]}; }}},
  ...
  return {
    ...
    stats() { return {stopped,audioClosed,micRequests}; },
  };
}
...
test('recognized answer survives a save failure and capture timer is cancelled', async () => {
  ...
  const ws = h.sockets[0]; ws.open();
  await ws.message({setupComplete:{}}); await flush();
  await ws.message({serverContent:{turnComplete:true}});
  await h.tick(80);
  h.button('답변 말하기').click(); await flush();
  assert.deepEqual(ws.sent.at(-1), {realtimeInput:{activityStart:{}}});
  await ws.message({serverContent:{inputTranscription:{text:'안전을 확보하고 보고합니다.'}}});
