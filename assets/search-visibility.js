(function(root){
  ...
  function drawNaver(){
    ...
    $('visibility-keyword-source').textContent=[selectedName()||'전체 지점',adCount?`인천 광고 계정 ON ${adCount}개 · 목록 ${date(current.keyword_source.updated_at)}`:'',regionalCount?`안양 지역 점검 키워드 ${regionalCount}개 · 광고 계정 연동 키워드 아님`:'',`최대 ${current.max_pages}페이지 확인`].filter(Boolean).join(' · ');
