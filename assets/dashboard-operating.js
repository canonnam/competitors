(function(root,factory){
  ...
  function render(record,expanded=false) {
    ...
    const data=record.data,view=prepare(data),{month,year,branches,rows}=view;
    if(!month)return '<p class="dash-empty">아직 등록된 운영비 자료가 없습니다.</p>';
    return `<div class="dash-operating-meta"><p class="dash-meta">운영 손익 · 단위: 만원</p><div class="dash-profit-legend">${branches.map(b=>`<span><i class="${b.id}" aria-hidden="true"></i>${esc(b.name)}</span>`).join('')}</div><p class="dash-meta">막대를 누르면 해당 월 분석으로 이동합니다.</p></div>${record.error?'<p class="dash-warning">연결 확인 필요 · 이전에 불러온 자료입니다.</p>':''}${plot(view)}<p class="dash-profit-mobile-hint dash-meta">좌우로 밀어 전체 월을 확인하세요.</p><div class="dash-operating-footer"><p class="dash-meta">${year}년 · 실제 입출금 기준 · 자료 기준 ${esc(data.sourceDate||'확인 필요')}</p>${rows.some(row=>row.profits.includes(null))?'<p class="dash-warning">자료가 없는 지점·월은 —로 표시하며 0원으로 간주하지 않습니다.</p>':''}<details${expanded?' open':''}><summary data-dash-key="operating-values">월별 수치 보기</summary><div class="dash-table-wrap"><table class="dash-branch-table"><caption class="dash-sr-only">지점별 월별 운영손익 · 단위: 만원</caption><thead><tr><th scope="col">월</th>${branches.map(b=>`<th scope="col">${esc(b.name)}</th>`).join('')}</tr></thead><tbody>${rows.map(row=>`<tr><th scope="row"><a href="${detailLink(row.month)}" data-dash-key="operating-table-${row.month}" aria-label="${monthName(row.month)} 상세 분석">${row.month.slice(2,4)}.${row.month.slice(5)}</a></th>${row.profits.map(profit=>`<td>${profit===null?'자료 없음':num(profit/10000)}</td>`).join('')}</tr>`).join('')}</tbody></table></div></details><p class="dash-footnote">등록된 월별 자료 기준이며 새 거래를 자동 수집하지 않습니다.</p></div>`;
  }
